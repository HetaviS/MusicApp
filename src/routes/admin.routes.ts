...
import {
  addOrRemoveSongsFromAlbumSchema,
  createAlbumSchema,
  updateAlbumSchema,
} from "../zod/album.validator";
import {
  addSongsToAlbum,
  createAlbum,
  getAllAlbums,
...
import {
  addOrRemoveSongsFromMovieSchema,
  createMovieSchema,
  updateMovieSchema,
} from "../zod/movie.validator";
import { createConfig, getConfig, updateConfig } from "../controllers/config.controller";
import { associateInclusionToPlan, createInclusion, createPlan, deleteInclusion, deletePlan, getAllInclusions, getAllPlans, updateInclusion, updatePlan } from "../controllers/subscription.controller";
...
router.post(
  "/add-data-to-block",
  validateBody(updateBlockDataSchema),
  addDataToBlock
);
router.post(
...
router.post(
  "/delete-artist",
  validateBody(
    z.object({ user_id: z.string().min(1, "user_id is required.") })
  ),
  deleteArtist
);
...
router.post(
  "/delete-genre",
  validateBody(
    z.object({ genre_id: z.string().min(1, "genre_id is required.") })
  ),
  removeGenre
);
...
router.post(
  "/add-Songs-to-album",
  validateBody(addOrRemoveSongsFromAlbumSchema),
  addSongsToAlbum
);
router.post(
...
router.post(
  "/add-songs-to-movie",
  validateBody(addOrRemoveSongsFromMovieSchema),
  addSongsToMovie
);
router.post(
  "/remove-songs-from-movie",
  validateBody(addOrRemoveSongsFromMovieSchema),
  removeSongsFromMovie
);
...
router.post('/create-subscription-plan', createPlan);
router.post('/update-subscription-plan', updatePlan);
router.post('/delete-subscription-plan', deletePlan);

router.post('/create-subscription-inclusion', createInclusion);
router.post('/update-subscription-inclusion', updateInclusion);
router.post('/delete-subscription-inclusion', deleteInclusion);

router.post('/associate-inclusion-to-plan', associateInclusionToPlan);
