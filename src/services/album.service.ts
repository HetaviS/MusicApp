...
import { song_service } from ".";

async function createAlbum(
  albumPayload: Partial<IAlbum>
): Promise<IAlbum | null> {
  try {
...
async function addSongsToAlbum(
  songIds: string[],
  albumId: string
): Promise<ISong[] | null> {
  try {
    const albumSongs = await Promise.all(
      songIds.map((song_id: string) =>
        AlbumSongs.create({ album_id: albumId, song_id: song_id })
      )
    );
...
async function removeSongsFromAlbum(
  songIds: string[],
  albumId: string
): Promise<number[] | null> {
  try {
    const albumSongs = await Promise.all(
      songIds.map(
        async (song_id: string) =>
          await AlbumSongs.destroy({
            where: { album_id: albumId, song_id: song_id },
          })
      )
    );
    return albumSongs;
...
async function updateAlbum(
  albumPayload: Partial<IAlbum>,
  where: Partial<IAlbum>
): Promise<IAlbum | null> {
  try {
    const album = await Album.update(albumPayload, {
      where: where,
      returning: true,
    });
...
async function isfavorite(userId: number, albumId: number): Promise<boolean> {
  try {
    const favorite = await favoriteAlbums.findOne({
  ...
  try {
    ...
    albums = await Promise.all(
      albums.map(async (album) => {
        ...
        const songs_count = await AlbumSongs.count({
          where: { album_id: rawAlbum.album_id },
        });
...
async function getFavoriteAlbums(
  ...
  try {
    ...
    let albums = await favoriteAlbums.findAll({
      where: { user_id: userId },
      attributes: ["album_id"],
      limit,
      offset,
