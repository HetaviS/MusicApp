...
async function register(req: Request, res: Response) {
    try {
        ...
        if (req.body.login_type == 'email') {
            ...
            if (user) {
                await user_service.updateUser({ otp: parseInt(otp) }, { email: req.body.email });
            ...
            let user = await user_service.getUser({ mobile_number: req.body.mobile_number });
            let otpSent;
            if (req.body.country_code == '+91') {
                otpSent = await sendMesg91TP(req.body.country_code, req.body.mobile_number, otp);
            }
            else {
                logger.info('Sending OTP via Twilio');
                otpSent = await sendTwilioOTP(req.body.country_code, req.body.mobile_number, otp);
            }
            if (user) {
                await user_service.updateUser({ otp: parseInt(otp) }, {  mobile_number: req.body.mobile_number });
                return response_service.successResponse(res, 'OTP sent successfully.', { newUser: false });
            }
            else {
                await user_service.createUser({ ...req.body, otp });
        ...
        else if (req.body.login_type == 'google'|| req.body.login_type == 'apple') {
            let user = await user_service.getUser({ email: req.body.email });
            
            if (user) {
                const token = await token_service.generateToken({user_id:user.user_id});
                return response_service.successResponse(res, 'Logged in successfully.', {user:removeExtraFields(user,['otp', 'login_verification_status', 'login_type', 'is_admin', 'device_token']),token,newUser:false });
            }
            else {
                const user = await user_service.createUser({ ...req.body });
                const token = await token_service.generateToken({user_id:user?.user_id});
                return response_service.successResponse(res, 'Logged in successfully.', { user:removeExtraFields(user,['otp', 'login_verification_status', 'login_type', 'is_admin', 'device_token']),token,newUser:true });
            }
        }
    }
    catch (err: any) {
