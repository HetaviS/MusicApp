...
async function updateArtist(req: Request, res: Response) {
    try {
        ...
        let artist = await user_service.getUser({ user_id: artistId, is_singer: true, is_deleted: false });
        let del_profile_pic;
        if (files && files['profile_pic']) {
            artistPayload.profile_pic = files['profile_pic'][0].path
            del_profile_pic = artist?.profile_pic;
        }
        artist = await user_service.updateUser({ ...artistPayload }, { user_id: artistId, is_singer: true, is_deleted: false });
        if (!artist) return response_service.badRequestResponse(res, 'Artist not found.');
...
async function deleteArtist(req: Request, res: Response) {
    try {
        const artistId = req.body.user_id;
        const artist = await user_service.updateUser({ is_deleted: true }, { user_id: artistId, is_singer: true, is_deleted: false });
        if (!artist) return response_service.badRequestResponse(res, 'Artist not found.');
